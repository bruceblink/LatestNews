...
Phase 2 的核心目标是把过多数据源转换成可扫描的阅读入口：先帮助用户判断“今天哪些内容值得看”，再进入具体来源或原文。洞察能力按“排序 -> 聚合 -> 解释 -> 过滤”建设，避免用户在源列表中逐个点开。

- 建立统一 Feed 页面：合并关注源、热门源和实时源，支持按时间、来源和分类过滤。
- 增加热点排行：按多源覆盖度、更新时间、来源权重、分类权重和阅读状态生成可解释的热点列表，缓解数据源过多导致的浏览压力。
- 增加话题事件追踪：把相近标题、相同 URL、连续更新和跨源报道聚合成话题，展示时间线、相关来源、首发时间、最新进展和代表标题。
- 增加统计洞察视图：展示来源活跃度、分类占比、更新频率、关键词趋势和词云图，帮助用户快速判断今天应该看什么。
- 增加词云与关键词联动：词云不只做展示，点击关键词后进入统一 Feed 过滤，并显示相关话题、来源分布和最近更新时间。
- 增加洞察解释信息：每个热榜、话题和关键词都展示命中来源数、最近更新时间、分类分布、已读影响和排序原因。
- 增强阅读状态：已读、稍后读、收藏、隐藏。
...
- 用户可以在一个页面内看到热点排行、话题事件、关键词趋势和来源健康概览。
- 用户可以从热榜、话题或词云直接跳转到过滤后的 Feed，而不是回到数据源列表手动查找。
- 热点和话题算法保持可解释、可测试，不把来源展示逻辑耦合进抓取层。
- 默认视图优先展示高置信、高覆盖和最近更新的内容，同时保留按来源浏览的入口。
...
6. Phase 2 洞察模型草案。
    ...
    - 第一阶段输出：热点排行、话题事件、词云关键词、来源活跃度和分类占比。
    - 验证：覆盖热点排行、话题聚合、关键词统计和分类统计的纯函数测试，运行 `pnpm exec vitest -c vitest.config.ts test/news-insights.test.ts`。
...
8. 统一 Feed 与洞察联动。
    - 文件范围：统一 Feed 页面、洞察页、前端筛选状态和共享 Query Key。
    - 验证：过滤参数单元测试、页面冒烟，以及 `/`、`/insights`、相关 API 的本地检查。
